<template>
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-input v-model="queryParams.query" :clearable="true" @keyup.enter.native="getUsersInfo" @clear="getUsersInfo" placeholder="请输入内容">
            <el-button
              slot="append"
              icon="el-icon-search"
              @click="getUsersInfo"
        ...
        <el-col :span="6">
          <el-button type="primary" @click="addUsersInfomation=!addUsersInfomation">添加用户</el-button>
        </el-col>
      </el-row>
      <!-- 用户列表数据加载区 -->
      <el-table :data="usersListInfo" border style="width: 100%">
        ...
        <el-table-column label="操作" width="230">
          <template slot-scope="info">
            <!-- 编辑按钮 -->
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="editUserInfo(info.row.id)"></el-button>
            <!-- 删除按钮 -->
            <el-button type="danger" icon="el-icon-delete" size="mini" @click="editUserDelete(info.row.id,info.row.username)"></el-button>
    ...
    <!-- 用户添加弹框 -->
    <template>
        <el-dialog title="添加用户" :visible.sync="addUsersInfomation" @close="addUserRest">
            <el-form :model="addUser" status-icon ref="addUserRef" :rules="addUserRules" label-width="100px" class="demo-ruleForm">
    ...
    <!-- 修改用户弹框 -->
    <template>
        <el-dialog title="修改用户" :visible.sync="editUserInfomation" @close="editUserRest">
            <el-form :model="editUser" status-icon ref="editUserRef" :rules="editUserRules" label-width="100px" class="demo-ruleForm">
              <el-form-item label="用户名" prop="username">
                <el-input v-model="editUser.username" autocomplete="off" :disabled="true"></el-input>
              </el-form-item>
              <el-form-item label="邮箱" prop="email">
...
export default {
  ...
  methods: {
    //   请求用户列表
      async getUsersInfo () {
          const {data:ret} = await this.$http.get('users',{params:this.queryParams});
          console.log(ret);
    ...
    // 编辑用户
    async editUsersInfo () {
      const {data:ret} = await this.$http.put(`users/${this.editUser.id}`,{email:this.editUser.email,mobile:this.editUser.mobile});
      console.log(ret);
      if(ret.meta.status != 200) {
        return this.$message.error(ret.meta.msg);
      }
